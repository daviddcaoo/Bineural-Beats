...
#http://paletton.com/#u1id=34T0u0kllllaFw0g0qFqFg0w0aF#
pg.init()
...
class Bebop(tk.Tk):
    ...
    def makeSong(self):
        ...
        os.system('python {} {}'.format(weightfile, str(songList)))

        #print(num_epochs)
        # rnn/rbm train model [rnn_rbm_train (num_epochs)]
        trainfile = (sys.path[0] + '\RNN\\rnn_rbm_train.py')
        os.system('python {} {} {}'.format(trainfile, num_epochs, songList))
        ...
        os.system('python {} {} {}'.format(generatefile, epochpath, songList))
...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        button = tk.Button(self, image=musicmakerButton, font=Rockwell, height=buttonHeight, width=buttonWidth, command=lambda: controller.show_frame(PageOne), fg=blueblack)
        ...
        button2 = tk.Button(self, image=jukeboxButton, font=Rockwell, height=buttonHeight, width=buttonWidth, command=lambda: controller.show_frame(PageTwo))
...
class PageOne(tk.Frame):
    def __init__(self, parent, controller):
        ...
        homebutton = tk.Button(self, height=50, borderwidth=1, width=100, image=homeImgMaker, command=lambda: controller.show_frame(StartPage))
        ...
        fileNameBox = tk.Listbox(self, width=50, height=20, fg=white, bg=blueblack, font=(LIST_FONT, 10), selectmode=tk.EXTENDED)
        fileNameBox.grid(row=1, column=1, rowspan=3)

        global addImg
        addImg = tk.PhotoImage(file="addbutton.png")
        openButton = tk.Button(self, height=buttonHeight, width=buttonWidth, image=addImg, font=Rockwell, command=lambda: controller.chooseFile(fileNameBox))
        ...
        directoryButton = tk.Button(self, height=buttonHeight, width=buttonWidth, image=deleteImg, font=Rockwell, command=lambda fileNameBox=fileNameBox: fileNameBox.delete(tk.ACTIVE))
        directoryButton.grid(row=2, column=3)

        #todo: make the dialogue box move to center
        global createImg
        createImg = tk.PhotoImage(file="makebutton.png")
        playButton = tk.Button(self, height=buttonHeight, width=buttonWidth, image=createImg, font=Rockwell, command=controller.makeSong)
        playButton.grid(row=3, column=3)

        #for scrolling, http://effbot.org/tkinterbook/listbox.htm
        #label = tk.Label(self, text="Song List")
...
class PageTwo(tk.Frame):
    def __init__(self, parent, controller):
        ...
        homebutton = tk.Button(self, height=50, borderwidth=1, width=100, image=homeImgJuke, command=lambda: controller.show_frame(StartPage))
        homebutton.grid(row=0, column=0)
        ...
        pauseButton = tk.Button(self, height=46, width=46, image=refreshImg, font=Rockwell, command=lambda: controller.updateJukeList(songFileNameBox))
        pauseButton.grid(row=1, column=0)

        songFileNameBox = tk.Listbox(self, width=50, height=20, fg=white, bg=blueblack, font=(LIST_FONT, 10), selectmode=tk.SINGLE)
        ...
        openButton = tk.Button(self, height=buttonHeight, width=buttonWidth, image=playImg, font=Rockwell, command=lambda: controller.playSong(songFileNameBox.get(songFileNameBox.curselection())))
        openButton.grid(row=1, column=3)

        global stopImg
        stopImg = tk.PhotoImage(file="stopbutton.png")
        quitSongButton = tk.Button(self, height=buttonHeight, width=buttonWidth, image=stopImg, font=Rockwell, command=lambda: controller.stopSong())
        quitSongButton.grid(row=2, column=3)

        #for scrolling, http://effbot.org/tkinterbook/listbox.htm
        #label = tk.Label(self, text="Your Created Songs List")
        #label.grid(row=3, column=1,pady=75)
